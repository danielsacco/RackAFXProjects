...
public:
	...
	bool isStereoOutput(ProcessFrameInfo& processFrameInfo);

	bool isStereoInput(ProcessFrameInfo& processFrameInfo);

	bool isMonoOutput(ProcessFrameInfo& processFrameInfo);

	bool isMonoInput(ProcessFrameInfo& processFrameInfo);
...
protected:
	FeedForwardDynamicsProcessor dynamicsProcessors[2];
...
private:
	...
	int hardLimitGate = 0;
	enum class hardLimitGateEnum { SWITCH_OFF,SWITCH_ON };	// to compare: if(compareEnumToInt(hardLimitGateEnum::SWITCH_OFF, hardLimitGate)) etc...

	int softKnee = 0;
	enum class softKneeEnum { SWITCH_OFF,SWITCH_ON };	// to compare: if(compareEnumToInt(softKneeEnum::SWITCH_OFF, softKnee)) etc...

	// --- Meter Plugin Variables
	float grMeter = 0.f;
	float detectorOutMeter = 0.f;
